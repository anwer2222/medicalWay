...
Srch.AU.get_querySuggestionLanguage = function Srch_AU$get_querySuggestionLanguage$st() {
    var $v_0 = -1;
    var $v_1 = Srch.ScriptApplicationManager.get_current();
    var $$dict_4 = $v_1.queryGroups;
    ...
    _display: function(completionItems, isQuery, lengthOfPrevSection) {
        ...
        for (var i = 0; i < completionItems.length; i++) {
            ...
            if (!isQuery) {
                ...
                resultAnchor.innerHTML = STSHtmlEncode(completionItems[i].get_title());
        ...
        var DateRangeExponentialBucketConfig = {
            ...
            AjaxControlToolkit.SliderRefinementControl.GetDefaultBuckets = function (ctx)
            {
                ...
                if (!$isNull(ctx.RefinementControl)) {
                    ...
                    if (specBoundaries.length > 1 && specBoundaries[0].indexOf("discretize=manual") != -1) {
                        // The first entry contains "discretize=manual" so entries 1 through n are boundary values
